...
export default class extends Controller {
  static targets = ["timeline", "selection", "modal", "habitSelect", "newHabitName", "startHourInput", "endHourInput"]
  static values = {
    date: String,
  ...
  buildHourSlotMap() {
    ...
    const slots = this.timelineTarget.querySelectorAll('.timeline-hour[data-hour]')
    slots.forEach(slot => {
  ...
  handleMouseMove(event) {
    ...
    this.selectedStartHour = Math.max(this.startHourValue, this.selectedStartHour)
  ...
  updateSelection() {
    ...
    const height = (endSlot.offsetTop + endSlot.offsetHeight) - startSlot.offsetTop
  ...
  openModal() {
    if (this.hasModalTarget) {
      ...
      if (timeDisplay) {
        timeDisplay.textContent = `${this.formatHour(this.selectedStartHour)} - ${this.formatHour(this.selectedEndHour)}`
      }
